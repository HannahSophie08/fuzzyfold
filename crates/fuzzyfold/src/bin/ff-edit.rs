...
fn main() -> Result<()> {
    ...
    let counts = edit_count(&sequence, &master, &zero_indexed_positions);
    ...
    let positions = cli.positions.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(",");
    writeln!(writer, "# positions={}", positions)?;
    write!(writer, "time,")?;
    writeln!(writer, "{}", positions)?;
    for (i, row) in counts.iter().enumerate() {
        let r = row.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(",");
        let t = times[i];
        writeln!(writer, "{},{}", t, r)?;
    }
...
fn edit_count (sequence: &NucleotideVec, master: &Vec<FxHashMap<DotBracketVec, usize>> , positions: &Vec<usize>) -> Vec<Vec<usize>> {

    master.iter().map(|counts| {
        let mut edit_counts = vec![0usize; positions.len()];
        for (db, count) in counts.iter() {  
            for (i, &position) in positions.iter().enumerate() {
                    if gets_edited(sequence, db, position) {
                        edit_counts[i] += count;
                    }
            }
        }
...
// checks whether position is paired with 'C' or 'U', and whether the 
// 5' and 3' neighbor are paired
fn gets_edited (sequence: &NucleotideVec, structure: &DotBracketVec, position: usize) -> bool {

    if position + 1 >= structure.len()  {
        return false; 
    }
    let pt = PairTable::try_from(structure).unwrap();
    let partner = match pt[position] {
        Some(p) => p as usize,
        None => return false,
    };
    let partner_base = sequence[partner];

    if partner_base != Base::C && partner_base != Base::U {
        return false 
    } 
    if !matches!(structure[position-1], DotBracket::Open | DotBracket::Close) {
        return false 
    } 
    if !matches!(structure[position+1],  DotBracket::Open | DotBracket::Close) {
        return false 
